...
export async function GET() {
  try {
    ...
    return new Response(JSON.stringify(mockOrderBook), {
      status: 200,
      headers: Object.fromEntries(headers)
    });
  } catch (error) {
    console.error('Error in mock-orderbook API:', error);
    return new Response(JSON.stringify({ error: 'Failed to generate order book data' }), {
      status: 500,
      headers: {
